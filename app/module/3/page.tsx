...
export default function Module3Page() {
  ...
  const inputClass = "w-full bg-gray-50 text-[#1A1F36] text-sm px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#F4B942]"
          ...
          <div className="bg-white rounded-2xl p-4 mb-6" style={{ border: '1px solid #e5e7eb' }}>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">Ebook</span>
                <span className="text-[#1A1F36] font-semibold">₱{parseInt(ebookValuePeso).toLocaleString()}</span>
              </div>
              {bonuses.map((b, i) => (
                <div key={i} className="flex justify-between">
                  <span className="text-gray-500 truncate pr-2">{b.bonus_name}</span>
                  <span className="text-[#1A1F36] font-semibold flex-shrink-0">₱{b.value_peso.toLocaleString()}</span>
                </div>
              ))}
              <div className="border-t border-gray-200 pt-2 flex justify-between">
                <span className="text-gray-500">Total Value</span>
                <span className="text-gray-500">₱{totalValue.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
  ...
  return (
    ...
    <div className="max-w-[430px] md:max-w-3xl mx-auto flex flex-col min-h-screen">
      ...
      <div className="px-4 pt-5 pb-3 flex items-center gap-3 bg-[#1A1F36]">
        <button onClick={goBack} className="text-[#F4B942] hover:text-white transition-colors">
          <BackIcon />
        </button>
        <div className="flex-1">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Module 3</p>
      ...
      {/* Error */}
      {error && (
        <div className="mx-4 mt-3 text-red-600 text-xs rounded-xl px-4 py-3" style={{ background: '#fef2f2', border: '1px solid #fecaca' }}>
          {error}
        </div>
      )}
      ...
      <div className="flex-1 px-4 pt-4 pb-36 overflow-y-auto">
        ...
        {step === 'foundation' && (
          <div className="space-y-4">
            <div>
              <p className="text-[#1A1F36] font-bold text-lg mb-1">Let&apos;s build your offer.</p>
              <p className="text-gray-500 text-sm">Here&apos;s what we know about you so far. Confirm these details before we start.</p>
            </div>

            <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #e5e7eb' }}>
              <p className={labelClass}>Your Ebook</p>
              <p className="text-[#1A1F36] text-sm font-semibold">{ebookTitle || 'No completed ebook found'}</p>
            </div>

            <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #e5e7eb' }}>
              <p className={labelClass}>Your Target Market</p>
              <p className="text-[#1A1F36] text-sm">{clarity?.target_market}</p>
            </div>

            <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #e5e7eb' }}>
              <p className={labelClass}>Problem You Solve</p>
              <p className="text-[#1A1F36] text-sm">{clarity?.core_problem}</p>
            </div>

            <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #e5e7eb' }}>
              <p className={labelClass}>Your Unique Approach</p>
              <p className="text-[#1A1F36] text-sm">{clarity?.unique_mechanism}</p>
            </div>

            <div className="rounded-xl px-4 py-3" style={{ background: '#fffbeb', border: '1px solid #F4B942' }}>
        ...
        {step === 'transformation' && (
          <div className="space-y-4">
            <div>
              <p className="text-[#1A1F36] font-bold text-lg mb-1">What result do they get?</p>
              <p className="text-gray-500 text-sm">Based on your ebook and clarity sentence, here&apos;s the transformation your buyer gets. Edit it if needed.</p>
            </div>
            ...
            {transformationLoading && (
              <div className="text-center py-14">
                ...
                <p className="text-sm font-medium text-[#1A1F36]">Writing your transformation statement…</p>
              </div>
            )}

            {!transformationLoading && transformation && !editingTransformation && (
              <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #F4B942' }}>
                <p className="text-[10px] font-bold text-[#F4B942] uppercase tracking-wide mb-2">Your Transformation Statement</p>
                <p className="text-[#1A1F36] text-sm leading-relaxed">{transformation}</p>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl font-semibold text-sm"
                    style={{ background: '#e5e7eb', color: '#1A1F36' }}
                  >
                    Use as written
                  </button>
                </div>
          ...
          <div className="space-y-4">
            ...
            {objectionsLoading && (
              <div className="text-center py-14">
                ...
                <p className="text-xs text-gray-500 mt-1">About 10 seconds</p>
              </div>
            )}
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div
                            ...
                            style={{
                              background: isSelected ? '#F4B942' : '#f3f4f6',
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">{obj.underlying_fear}</p>
                          </div>
                        </div>
                      </button>
                    )
        ...
        {step === 'bonuses' && (
          <div>
            <p className="text-[#1A1F36] font-bold text-lg mb-1">Build your bonus stack.</p>
            <p className="text-gray-500 text-sm mb-4">Each bonus answers one of your buyer&apos;s doubts. All formats are text-based documents — no audio or video.</p>
                ...
                <div key={i} className="bg-white rounded-2xl p-4" style={{ border: '1px solid #e5e7eb' }}>
                  {bonus.loading ? (
                    <div className="flex items-center gap-3 py-2">
                      ...
                      <p className="text-gray-500 text-xs">Creating bonus for: &ldquo;{bonus.objection_addressed.substring(0, 50)}…&rdquo;</p>
                    </div>
                  ) : (
                    <div>
                      <div className="flex items-start justify-between gap-2 mb-3">
                        <div className="flex-1">
                          ...
                          <p className="text-[11px] text-gray-500 italic">&ldquo;{bonus.objection_addressed}&rdquo;</p>
                        </div>
                        <button
        ...
        {step === 'price_anchor' && (
          <div className="space-y-4">
            <div>
              <p className="text-[#1A1F36] font-bold text-lg mb-1">Price your offer.</p>
              <p className="text-gray-500 text-sm">Set the perceived value of everything included and your selling price. Then let AI write the justification.</p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #e5e7eb' }}>
              ...
              {bonuses.map((b, i) => (
                <div key={i} className="flex justify-between items-center mb-2 last:mb-0">
                  <span className="text-gray-500 text-sm truncate pr-2">{b.bonus_name}</span>
                  <span className="text-[#1A1F36] text-sm font-semibold flex-shrink-0">₱{b.value_peso.toLocaleString()}</span>
                </div>
              ))}
              <div className="border-t border-gray-200 mt-3 pt-3 flex justify-between">
                <span className="text-gray-500 text-sm font-semibold">Total Value</span>
            ...
            {priceJustLoading && (
              <div className="text-center py-4">
                ...
                <p className="text-xs text-gray-500">Writing your price justification…</p>
              </div>
            )}
            ...
            {priceJustification && (
              <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #F4B942' }}>
                ...
                <p className="text-[#1A1F36] text-sm leading-relaxed">{priceJustification}</p>
                <button
                  onClick={() => { setPriceJustification(''); handleGeneratePriceJustification() }}
                  className="flex items-center gap-1.5 mt-3 text-xs text-gray-500 font-semibold"
                >
                  <RefreshIcon /> Regenerate
                </button>
              </div>
        ...
        {step === 'guarantee' && (
          <div className="space-y-4">
            <div>
              <p className="text-[#1A1F36] font-bold text-lg mb-1">Remove their risk.</p>
              <p className="text-gray-500 text-sm">A guarantee tells your buyer: &ldquo;I believe in this so much, I&apos;m taking the risk for you.&rdquo; Pick one or write your own.</p>
            </div>
            ...
            <div className="space-y-2">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-sm text-[#1A1F36]">{g}</p>
                    </div>
                  </button>
                )
              })}
              ...
              <button
                ...
                className="w-full text-left rounded-xl p-3 transition-all"
                style={{
                  background: customGuarantee ? '#fffbeb' : '#ffffff',
                  border: `2px solid ${customGuarantee ? '#F4B942' : '#e5e7eb'}`,
                }}
              >
                <div className="flex items-center gap-3">
                  <div
                    className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                    style={{
                      background: customGuarantee ? '#F4B942' : '#f3f4f6',
                      border: `2px solid ${customGuarantee ? '#F4B942' : '#d1d5db'}`,
                    }}
                  >
                    {customGuarantee && <div className="w-2 h-2 rounded-full bg-[#1A1F36]" />}
                  </div>
                  <p className="text-sm text-[#1A1F36]">Write my own guarantee</p>
                </div>
              </button>
            </div>
        ...
        {step === 'offer_statement' && (
          <div className="space-y-4">
            <div>
              <p className="text-[#1A1F36] font-bold text-lg mb-1">Your Irresistible Offer.</p>
              <p className="text-gray-500 text-sm">We&apos;ll now put everything together into one clear, compelling offer statement you can be proud of.</p>
            </div>
            ...
            {!offerStatement && !offerLoading && (
              <div className="bg-white rounded-2xl p-4 space-y-2" style={{ border: '1px solid #e5e7eb' }}>
                ...
                <p className="text-sm text-gray-700"><span className="text-gray-500">Ebook:</span> {ebookTitle}</p>
                <p className="text-sm text-gray-700"><span className="text-gray-500">For:</span> {clarity?.target_market}</p>
                <p className="text-sm text-gray-700"><span className="text-gray-500">Transformation:</span> {transformation}</p>
                <p className="text-sm text-gray-700"><span className="text-gray-500">Bonuses:</span> {bonuses.length} included</p>
                <p className="text-sm text-gray-700"><span className="text-gray-500">Price:</span> ₱{parseInt(sellingPrice).toLocaleString()} (worth ₱{totalValue.toLocaleString()})</p>
                <p className="text-sm text-gray-700"><span className="text-gray-500">Guarantee:</span> {guarantee}</p>
              </div>
            )}
            ...
            {offerLoading && (
              <div className="text-center py-14">
                <div className="w-10 h-10 border-4 border-[#F4B942] border-t-transparent rounded-full animate-spin mx-auto mb-4" />
                <p className="text-sm font-medium text-[#1A1F36]">Building your irresistible offer…</p>
                <p className="text-xs text-gray-500 mt-1">This takes about 10 seconds</p>
              </div>
            )}
            ...
            {offerStatement && !offerLoading && (
              <div className="bg-white rounded-2xl p-4" style={{ border: '1px solid #F4B942' }}>
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 mt-4 text-xs text-gray-500 font-semibold"
                >
                  <RefreshIcon /> Regenerate
                </button>
              </div>
      ...
      {/* ── Bottom CTA ────────────────────────────────────────────────────── */}
      <div
        className="fixed bottom-0 w-full px-4 py-4 bg-white border-t border-gray-200"
